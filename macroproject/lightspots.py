...
class LightSpotModel:
    ...
    def point2line(self, point, p1, p2):
        ...
        k = p2 - p1
        s = np.cross(v, k)
        res = np.linalg.norm(s)
        if np.linalg.norm(k) == 0:
            res = 0
        else:
            res /= np.linalg.norm(k)
        return res
    ...
    def roughfindAllSpots(self, minAngle = 0, maxAngle = 90, astep = 1, absF = 0.001,
                          xmin = 0, xmax = 20, ymin = - 15, ymax = 15, 
                          zmin = -5, zmax = 5, size = 0.5, step = 1, minrays = 10):
        '''
        Функция производит грубый поиск 'пятен', в которые сфокусированы лучи, 
        при заданном диапазоне углов падения солнечного света
        ...
        pdData = pd.DataFrame(npData, columns = ['Angle', 
                                                 'MaxRays',
                                                 'X', 'Y', 'Z'])
        pdData.to_csv(self.modelname + '\\Rough_results' + str(minAngle) + '_' + str(maxAngle) + '.csv', sep=';') 
        return angles, maxrays, maxspots 
    
     
    
    def accuratefindAllSpots(self, fname, absF = 0.001, srange = [0.5, 0.5, 0.0], size = 0.1, 
                             step = 1, minrays = 30):
        '''
        Функция производит точный поиск 'пятен', в которые сфокусированы лучи, 
        при заданном диапазоне углов падения солнечного света
        absF - коэффициент поглощения
        srange - размер области точного поиска
        size - радиус 'пятна'
        step - шаг (в радиусах 'пятна') между соседними 'ячейками' поиска
        minrays - минимальная кратность превышения 'плотности' лучей в 'пятне' 
        отсекающего области фокусировки по сравнению с исходной
        ...
        p = np.array([np.array(data['X']), np.array(data['Y']), np.array(data['Z'])]).transpose()        
        #цикл перебора углов
        ...
        pdData = pd.DataFrame(npData, columns = ['Angle', 
                                                 'MaxFlowDens',
                                                 'X', 'Y', 'Z'])
        pdData.to_csv(self.modelname + '\\FinalSpot_results.csv', sep=';') 
        return angles, maxrays, maxspots
    ...
    def results2vtk(self, angle, spot):
        ...
        writer.SetFileName(self.modelname + '\spot_' + str(int(angle)) + '.vtu')
        writer.Write()
